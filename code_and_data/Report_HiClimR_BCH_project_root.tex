...
\NormalTok{  est,}
  ...
  \AttributeTok{ssc =} \FunctionTok{ssc}\NormalTok{(}
    \AttributeTok{K.adj =} \ConstantTok{FALSE}\NormalTok{,}
...
correction, and which degrees of freedom to use for scalar tests. The
relevant BCH correction for scalar inference is the group correction:
applying \(G/(G-1)\) to the clustered covariance is equivalent to
multiplying the raw CCE statistic by \(\sqrt{(G-1)/G}\) and comparing it
with a Student distribution with \(G-1\) degrees of freedom.

This is why the coefficient-count correction is set to false. BCH note
that Stata 10's clustered variance differs slightly from their exact
statistic because it also multiplies the covariance matrix by
\((N-1)/(N-k)\). They argue that Stata's reported clustered
\(t\)-statistics can be used directly in practice because this factor is
close to one and conservative. In our R implementation, however, the
objective is to reproduce the BCH statistic rather than the additional
Stata small-sample convention. We therefore keep the group correction
on, set the coefficient-count correction off, and use \(G-1\) degrees of
freedom for scalar tests. For joint Wald restrictions, BCH derive a
