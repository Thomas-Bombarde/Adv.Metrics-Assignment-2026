...
This is one way to handle spatial dependence, not the only one.
Conley-type standard errors model covariance as local in geographic
distance and are natural when dependence decays smoothly with distance.
The SPUR and SCPC approach of
\citet{mueller_watson_2022,mueller_watson_2024} and
\citet{becker_boll_voth_2026} asks a different question: whether spatial
persistence is so strong that levels regressions may be spurious, and
whether variables should be transformed before spatial-correlation
robust inference. Our HiClimR-BCH strategy sits between these
approaches. It does not estimate a distance kernel, and it does not test
for spatial unit roots. It builds large climate regions from treatment
co-movement, then asks whether the resulting regression scores behave
like a small number of approximately independent group sums.
...
The application follows the data design in \citet{burke2024}, and the
specification detailed in \citet{hsiang_burke_miguel_2013}, with the
following logic. A cell that is hotter than usual in a given month is
closer to a natural experiment than a comparison of hot and cold places.
The climate-conflict relationship is instead probed by deviations from
each cell's monthly climatology, with cell and year-month fixed effects.
Climate deviations may be exogenous, but they are not spatially
independent. Heat anomalies often cover many neighboring cells, and
conflict can also co-move within regions. Measuring uncertainty deals
with clustering grid-cell across of a range of options. Country clusters
are meaningful but may not match weather systems. Supranational
macroregions are closer to geographers' regional classifications, but
still impose boundaries rather than learning them from climate dynamics.
The question is therefore: what is the relevant spatial unit of
climate-conflict variation?

We use data-driven group definitions for three reasons. First,
temperature anomalies follow a physical process, so their spatial
co-movement can be measured before estimating any conflict model.
Second, the panel observes conflict at the grid-cell level, while
weather shocks operate over broader spatial systems; this mismatch
confuses, to us, the roadmap for inference. Third, cell and year-month
fixed effects remove permanent local differences and common monthly
shocks, so the remaining dependence is not mainly about levels, but
about whether residual regression scores still co-move across space.
...
Our aim is therefore not to replicate the main estimates in
\citet{burke2024}. We use their weather-conflict panel as a setting in
which the relevant spatial groups are genuinely ambiguous. The exercise
asks whether the number of groups changes fixed-(G) inference, and
whether data- driven climate regions produce more defensible group
scores than administrative boundaries. The empirical application is thus
mainly a test of group construction: how much inference changes when
independent information is defined by climate co-movement rather than by
countries or arbitrary grid cells.
...
monthly temperature data to Africa, assigns cells to countries using
Natural Earth polygons, spatially joins conflict events to cells,
expands events to the months they cover, and creates monthly conflict
counts and indicators. We retain the public weather-conflict fields and
exclude the Atlas wealth and WDI income variables used for the paper's
moderation analysis.

The resulting object is a monthly panel of \(2,695\) African grid cells
from 1989 to 2019. Each cell is matched to country identifiers,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{est }\OtherTok{\textless{}{-}} \FunctionTok{feols}\NormalTok{(conflict\_0\_1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ temp\_anom }\SpecialCharTok{|}\NormalTok{ cell\_id }\SpecialCharTok{+}\NormalTok{ ym, }\AttributeTok{data =}\NormalTok{ df)}
\FunctionTok{summary}\NormalTok{(}
\NormalTok{  est,}
  \AttributeTok{vcov =} \SpecialCharTok{\textasciitilde{}}\NormalTok{ bch\_region,}
  \AttributeTok{ssc =} \FunctionTok{ssc}\NormalTok{(}
    \AttributeTok{K.adj =} \ConstantTok{TRUE}\NormalTok{,}
    \AttributeTok{K.fixef =} \StringTok{"nonnested"}\NormalTok{,}
    \AttributeTok{G.adj =} \ConstantTok{TRUE}\NormalTok{,}
    \AttributeTok{t.df =} \StringTok{"min"}
\NormalTok{  )}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The first line estimates the fixed-effects regression. The covariance
formula requests one-way clustering by the candidate climate region. The
small-sample correction options specify whether to adjust for estimated
coefficients, how to count fixed effects, whether to apply a group-count
correction, and which degrees of freedom to use for scalar tests.
Setting the degrees of freedom rule to the minimum cluster count makes
the scalar reference distribution use \(G-1\) degrees of freedom in this
one-way clustering setting. For joint Wald restrictions, BCH derive a
scaled \(F\) reference; ordinary regression summaries do not
automatically impose that scaling.
...
close to independent. This approach is most appropriate when the
researcher can defend a small number of substantively meaningful
dependence regions. Conley inference is more appropriate when the
relevant assumption is smooth distance decay. SPUR and SCPC are more
appropriate when the concern is strong low-frequency spatial persistence
or spatial unit roots. These approaches can be complementary: SPUR-style
diagnostics can assess whether transformation is needed, Conley or SCPC
can handle residual weak spatial dependence, and BCH-style grouping can
discipline inference when the credible independent information is a
small number of large regions. In this application, the positive
